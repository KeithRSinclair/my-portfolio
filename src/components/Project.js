            ...
            <section className="grid grid-cols-2 gap-8">
                {projectData && projectData.map((project, index) => (
                    <article className="relative rounded-lg shadow-xl bg-white p-16">
                        <h3 className="text-gray-800 text-3xl font-bold mb-2 hover:text-green-700"><a href={project.link} alt={project.title} target="_blank"  >{project.title}</a></h3>
                        <div className="text-gray-500 text-xs space-x-4">
                            ...
                            <a href={project.link} alt={project.title} target="_blank" className="text-blue-700 font-bold hover:underline hover:text-green-700">View Project {" "}<span role="img" aria-label="right pointer">➡</span></a>
